...
def attach_simclr_head(base_model, hidden_1=256, hidden_2=128, hidden_3=64):
    """
...
def create_linear_model_from_base_model(base_model, output_shape, intermediate_layer=7, hidden_size=128, added_layers=3):
    ...
    for layer in range(added_layers-1):
        ...
        x = tf.keras.layers.Dropout(0.5, name='drop_{}'.format(layer))(x)  
    # last layer
...
def create_full_classification_model_from_base_model(base_model, output_shape, intermediate_layer=7,
                                                    first_last=False, middle=False):
    """
    ...
    #x = tf.keras.layers.Dense(128, activation='relu', 
                    #kernel_regularizer=tf.keras.regularizers.l2(l=1e-4))(intermediate_x)
    x = tf.keras.layers.Dense(128, activation='relu', 
    ...
    if first_last:
        ...
        for l in [3, 4]: #the middle CONV1D layer is frozen
            model.layers[l].trainable = False
